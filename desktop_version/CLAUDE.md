...
The desktop half of the digit-recognition-study project: a minimal, two-script handwritten digit recognizer. Train an MLP classifier on MNIST, then draw a digit in a Tkinter GUI and have it predict what you drew. No package manager config, no tests, no build system — just plain Python scripts run directly.

The web version lives alongside this one in `../web_version/` (see its own `CLAUDE.md`); the two are independent deployments and don't share code, only the same MNIST-trained-model concept.
...
Run these from inside `desktop_version/`:
...
- **`digit_model.joblib`** — Trained model artifact loaded directly by `draw_app.py`, gitignored (not committed) — regenerate by rerunning `train_model.py` before first launching the app.
...
When touching prediction accuracy issues, look here first before touching the model architecture in `train_model.py`. If a web version ever needs the same accuracy, this same pipeline should be ported rather than reinvented.
